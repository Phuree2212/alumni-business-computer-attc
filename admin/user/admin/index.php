...
if ($_SERVER['REQUEST_METHOD'] === 'GET' && (!empty($keyword) || !empty($start_date) || !empty($end_date))) {
    ...
    $totalItems = $admin->getSearchAndFilterCount($keyword,  $start_date, $end_date);

    // สร้าง pagination
    $pagination = new PaginationHelper($currentPage, $itemsPerPage, $totalItems);

    // ดึงข่าวตามเงื่อนไข
    $admin_list = $admin->searchAndFilter($keyword,  $start_date, $end_date, $pagination->getLimit(), $pagination->getOffset());
} else {
    // นับจำนวนรายการทั้งหมด
    $totalItems = $admin->getTotalCount();
...
<body>
    ...
    <div class="main-content">
        <!-- Heading -->
        <div class="d-flex">
            <h3 class="table-header mb-5"><b>จัดการข้อมูลผู้ดูแลระบบ</b></h3>
            <div>
                <a href="add.php" class="btn btn-success mx-3">เพิ่มข้อมูลผู้ดูแลระบบ</a>
            </div>
        </div>
        ...
        <div class="table-container">
            <div class="table-responsive">
                    ...
                    <tbody>
                        ...
                        <?php if (!empty($admin_list)) {
                            ...
                            foreach ($admin_list as $item) {
                                ...
                                $isset_image = $image != '' ? '../../../assets/images/user/admin/' . $image : '../../../assets/images/user/no-image-profile.jpg';
                        ?>
                                <tr>
                                    <td><?= $i++; ?></td>
                ...
                <?php if (empty($admin_list)) { ?>
                    <div class="text-center text-danger">ไม่พบข้อมูลผู้ดูแลระบบ</div>
                <?php } ?>
        ...
        <div class="modal-dialog modal-dialog-centered modal-lg">
            <div class="modal-content">
                ...
                <div class="modal-header">
                    <h5 class="modal-title text-warning" id="modalTitle">แก้ไขข้อมูล ผู้ดูแลระบบ</h5>
