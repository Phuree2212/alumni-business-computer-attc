...
if ($_SERVER['REQUEST_METHOD'] === 'GET' && isset($_GET['id'])) {
    ...
    $item = $report->getTopicReportProblem($id);
    if ($item) {
        //ข้อมูลกระทู้
        ...
        $images_array = explode(",", $item['image']);

        $post_student_code = $item['post_student_code'];
        $post_user_type = $item['post_user_type'];

        $post_by_id = $item['post_by_id'];
        $post_by = $item['post_first_name'] . ' ' . $item['post_last_name'];
...
<body>
    ...
    <div class="main-content">
        <div class="card-body">
            <h3 class="h3 mb-4 text-danger">รายละเอียดการรายงานกระทู้จากผู้ใช้งาน</h3>
            ...
            <div class="mb-3">
                <label class="form-label fw-bold">สถานะการตรวจสอบ<span class="text-danger"> : </span></label>
                <span class="info-value"><?php echo $status_string ?></span>
            </div>
            <hr>
            ...
            <div class="mb-3">
                ...
                <span class="info-value"><?php echo $post_by_id ?></span> <a href="../../user/<?php echo $post_user_type ?>/?keyword=<?php echo $post_student_code ?>">ดูข้อมูลผู้โพสต์</a>
            </div>
            <div class="mb-3">
                <label class="form-label fw-bold">ผู้โพสต์กระทู้<span class="text-danger"> : </span></label>
                <span class="info-value"><?php echo $post_by ?></span>
            </div>
            <div class="mb-3">
                <label class="form-label fw-bold">หัวข้อ<span class="text-danger"> : </span></label>
                <span class="info-value"><?php echo $title ?></span>
            </div>
            <div class="mb-3">
                <label class="form-label fw-bold">เนื้อหากระทู้<span class="text-danger"> : </span></label>
                <span class="info-value"><?php echo strip_tags($content) ?></span>
            </div>
            <div class="mb-3">
                <label class="form-label fw-bold">รูปภาพกระทู้<span class="text-danger"> : </span></label><br>
                <?php if (!empty($images_array[0])) { ?>
                    <?php foreach ($images_array as $image) { ?>
                        <img src="../../../assets/images/webboard/<?php echo $image ?>" class="img-thumbnail">
                    <?php }
                } else { ?>
                    <span class="info-value">ไม่มีรูปภาพ</span>
                <?php } ?>
            </div>
            ...
            <div class="mt-3">
                ...
                <button onclick="deleteData(
                                    <?php echo $post_id ?>, 
                                    '<?php echo 'id=' . urlencode($post_id) . '&image=' . urlencode($string_image); ?>', 
                                    '../../webboard/delete.php'
